...
const Particles = ({
  className = "",
  quantity = 250, // Reduced default for performance with line-drawing
  staticity = 50,
  ease = 40,
  refresh = false,
  color = "#ffffff", // Added customizable color
  vx = 0.5, // Base velocity X
  vy = 0.5, // Base velocity Y
}) => {
  ...
  // Hex to RGB helper for dynamic alpha
  const hexToRgb = (hex) => {
    ...
    for (let i = 0; i < circles.current.length; i++) {
      for (let j = i + 1; j < circles.current.length; j++) {
        ...
        if (dist < maxDistance) {
          ...
          context.current.strokeStyle = `rgba(${rgbColor}, ${alpha})`;
  ...
  const animate = () => {
    ...
    // 1. Draw Lines first (so they appear behind particles)
    drawLines();
...
//     const translateX = 0;
//     const translateY = 0;
//     const size = Math.floor(Math.random() * 3) + 0.8;
//     const alpha = 0;
//     const targetAlpha = parseFloat((Math.random() * 0.6 + 0.1).toFixed(1));
//     const dx = (Math.random() - 0.5) * 0.2;
//     const dy = (Math.random() - 0.5) * 0.2;
//     const magnetism = 0.8 + Math.random() * 6;
//     return {
//       x,
//       y,
//       translateX,
...
//       context.current.fillStyle = `rgba(55, 255, 255, ${alpha})`;
//       context.current.fill();
